...
namespace IronPython.Runtime.Operations {
    internal static class IListOfByteOps {
        ...
        internal static Traffy.Objects.TrList/*!*/ RightSplit(this IList<byte>/*!*/ bytes, IList<byte>/*!*/ sep, int maxsplit, Func<IList<byte>/*!*/, Traffy.Objects.TrObject>/*!*/ ctor) {
            ...
            if (resultlen != 0) {
                ...
                foreach (var s in temp.container)
                    ret.container.Add(ctor(((IList<byte>)s.Native).ReverseBytes()));
            } else {
                ret = temp;
            }
            return ret;
        ...
        internal static Traffy.Objects.TrList/*!*/ SplitLines(this IList<byte>/*!*/ bytes, bool keepends, Func<List<byte>/*!*/, Traffy.Objects.TrObject>/*!*/ ctor) {
            Traffy.Objects.TrList ret = new Traffy.Objects.TrList();
            int i, linestart;
            for (i = 0, linestart = 0; i < bytes.Count; i++) {
                if (bytes[i] == '\n' || bytes[i] == '\r') {
                    ...
                    if (i < bytes.Count - 1 && bytes[i] == '\r' && bytes[i + 1] == '\n') {
                        if (keepends)
                            ret.container.Add(ctor(bytes.Substring(linestart, i - linestart + 2)));
                        else
                            ret.container.Add(ctor(bytes.Substring(linestart, i - linestart)));
                        linestart = i + 2;
                        i++;
                    } else { //'\r', '\n', or unicode new line as end of line marker
                        if (keepends)
                            ret.container.Add(ctor(bytes.Substring(linestart, i - linestart + 1)));
                        else
                            ret.container.Add(ctor(bytes.Substring(linestart, i - linestart)));
                        linestart = i + 1;
                    }
        ...
        internal static Traffy.Objects.TrList/*!*/ Split(this IList<byte>/*!*/ bytes, IList<byte> sep, int maxsplit, Func<List<byte>/*!*/, Traffy.Objects.TrObject>/*!*/ ctor) {
            ...
            if (sep == null) {
                if (maxsplit == 0) {
                    ...
                    result.container.Add(ctor(bytes.LeftStrip() ?? bytes as List<byte> ?? new List<byte>(bytes)));
                    return result;
                }
        ...
        internal static Traffy.Objects.TrList/*!*/ SplitInternal(IList<byte>/*!*/ bytes, byte[] seps, int maxsplit, Func<List<byte>/*!*/, Traffy.Objects.TrObject>/*!*/ ctor) {
            Debug.Assert(ctor != null);
        ...
        internal static Traffy.Objects.TrList/*!*/ SplitInternal(this IList<byte>/*!*/ bytes, IList<byte>/*!*/ separator, int maxsplit, Func<List<byte>/*!*/, Traffy.Objects.TrObject>/*!*/ ctor) {
            Debug.Assert(ctor != null);
        ...
        private static Traffy.Objects.TrList/*!*/ SplitEmptyString(bool separators, Func<List<byte>/*!*/, Traffy.Objects.TrObject>/*!*/ ctor) {
            Debug.Assert(ctor != null);

            Traffy.Objects.TrList ret = PythonOps.MakeEmptyList(1);
            if (separators) {
                ret.container.Add(ctor(new List<byte>(0)));
            }
            return ret;
        }
        ...
        internal static List<byte> Slice(this IList<byte>/*!*/ bytes, Traffy.Objects.TrSlice/*!*/ slice) {
            if (slice == null) {
                throw PythonOps.TypeError("indices must be slices or integers");
            }

            int start, stop, step;
            (start, stop, step) = slice._indices(bytes.Count);
            if (step == 1) {
